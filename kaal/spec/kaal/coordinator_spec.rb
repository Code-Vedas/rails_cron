...
RSpec.describe Kaal::Coordinator do
  ...
  describe '#calculate_and_dispatch_due_times' do
    it 'logs valid cron occurrences' do
      entry = instance_double(Kaal::Registry::Entry, key: 'job', cron: '* * * * *', enqueue: kw_enqueue)
    ...
    it 'logs warning when cron is invalid' do
      entry = instance_double(Kaal::Registry::Entry, key: 'job', cron: 'invalid_cron_xyz', enqueue: kw_enqueue)
  ...
  describe '#dispatch_if_due' do
    ...
    it 'acquires lock when fire_time is in the past' do
      entry = instance_double(Kaal::Registry::Entry, key: 'job', enqueue: kw_enqueue)
      adapter = double
    ...
    it 'rescues StandardError without logging when logger is nil' do
      configuration.logger = nil
      entry = instance_double(Kaal::Registry::Entry, key: 'job', enqueue: kw_enqueue { raise 'Error' })
      adapter = double
      configuration.backend = adapter
      allow(adapter).to receive(:acquire).and_return(true)
  ...
  describe '#dispatch_work' do
    ...
    it 'rescues StandardError and logs it' do
      entry = instance_double(Kaal::Registry::Entry, key: 'job', enqueue: kw_enqueue { raise 'Enqueue error' })
    ...
    it 'rescues StandardError without logging when logger is nil' do
      configuration.logger = nil
      entry = instance_double(Kaal::Registry::Entry, key: 'job', enqueue: kw_enqueue { raise 'Dispatch error' })
  ...
  describe 'Integration Tests for Full Coverage' do
    ...
    it 'calculate_and_dispatch_due_times with empty registry does nothing' do
      registry_empty = Kaal::Registry.new
    ...
    it 'dispatch_work generates correct idempotency_key' do
      ...
      entry = instance_double(Kaal::Registry::Entry, key: 'test-job', enqueue: enqueue_proc)
    ...
    it 'dispatch_work passes fire_time and idempotency_key to enqueue' do
      ...
      entry = instance_double(Kaal::Registry::Entry, key: 'job', enqueue: enqueue_proc)
      fire_time = Time.now
    ...
    it 'calculate_and_dispatch_due_times with occurrences executes dispatch loop' do
      call_count = [0]
      entry = instance_double(Kaal::Registry::Entry, key: 'job', cron: '* * * * *', enqueue: kw_enqueue { call_count[0] += 1 })
      adapter = double
      configuration.backend = adapter
    ...
    it 'dispatch_work with enqueue exception message' do
      ...
      entry = instance_double(Kaal::Registry::Entry, key: 'job', enqueue: kw_enqueue { raise error_msg })
    ...
    it 'calculate_and_dispatch_due_times with no matching window' do
      ...
      entry = instance_double(Kaal::Registry::Entry, key: 'yearly_job', cron: '0 0 1 1 *', enqueue: kw_enqueue)
    ...
    it 'dispatch_work logs debug when logger present' do
      entry = instance_double(Kaal::Registry::Entry, key: 'debug-job', enqueue: kw_enqueue)
    ...
    it 'dispatch_if_due acquires correct lock key' do
      ...
      configuration.backend = adapter
      allow(adapter).to receive_messages(acquire: true, call: true)
      entry = instance_double(Kaal::Registry::Entry, key: 'test', enqueue: kw_enqueue)
    ...
    it 'calculate_and_dispatch_due_times logs debug with real logger and cron' do
      ...
      entry = instance_double(Kaal::Registry::Entry, key: 'minute-job', cron: '* * * * *', enqueue: kw_enqueue)
  ...
  describe '#recover_entry' do
    let(:entry) { instance_double(Kaal::Registry::Entry, key: 'test_job', cron: '* * * * *', enqueue: kw_enqueue) }
    let(:recovery_start) { Time.current - 3600 }
    let(:recovery_end) { Time.current }

    it 'returns 0 when cron expression cannot be parsed' do
      invalid_entry = instance_double(Kaal::Registry::Entry, key: 'bad_job', cron: 'invalid')
