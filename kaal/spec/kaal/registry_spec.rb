...
RSpec.describe Kaal::Registry do
  ...
  describe '#add' do
    ...
    it 'raises when key is already registered' do
      ...
      expect { registry.add(key: test_key, cron: '0 10 * * *', enqueue: test_enqueue) }.to raise_error(Kaal::RegistryError, /already registered/)
    end
  ...
  describe '#inspect' do
    it 'returns a string representation' do
      registry.add(key: 'job1', cron: '0 9 * * *', enqueue: test_enqueue)
      registry.add(key: 'job2', cron: '0 10 * * *', enqueue: test_enqueue)
      expect(registry.inspect).to include('Kaal::Registry', 'size=2', 'job1', 'job2')
    end
