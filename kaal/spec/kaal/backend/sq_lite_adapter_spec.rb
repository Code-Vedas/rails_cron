...
RSpec.describe Kaal::Backend::SQLiteAdapter do
  let(:adapter) { described_class.new }

  before do
    Kaal::CronLock.delete_all
  end
  ...
  describe '#acquire' do
    ...
    it 'creates a lock record in database' do
      ...
      lock = Kaal::CronLock.find_by(key: 'test:key:456')
    ...
    it 'treats RecordNotUnique contention as not acquired' do
      allow(Kaal::CronLock).to receive(:create!).and_raise(ActiveRecord::RecordNotUnique.new('duplicate key'))
      allow(Kaal::CronLock).to receive(:cleanup_expired)

      result = adapter.acquire('test:key:db-unique', 60)

      expect(result).to be(false)
      expect(Kaal::CronLock).to have_received(:cleanup_expired).once
    end

    it 'treats wrapped uniqueness StatementInvalid as not acquired' do
      allow(Kaal::CronLock).to receive(:create!).and_raise(ActiveRecord::StatementInvalid.new('UNIQUE constraint failed'))
      allow(Kaal::CronLock).to receive(:cleanup_expired)
    ...
    it 'raises LockAdapterError for non-contention StatementInvalid failures' do
      allow(Kaal::CronLock).to receive(:create!).and_raise(ActiveRecord::StatementInvalid.new('syntax error'))

      expect do
        adapter.acquire('test:key:statement-error', 60)
      end.to raise_error(Kaal::Backend::LockAdapterError, /SQLite acquire failed/)
    end

    it 'raises LockAdapterError for non-unique constraint StatementInvalid failures' do
      allow(Kaal::CronLock).to receive(:create!).and_raise(ActiveRecord::StatementInvalid.new('CHECK constraint failed'))
    ...
    it 'stores correct TTL expiration time' do
      ...
      lock = Kaal::CronLock.find_by(key: 'test:key:ttl')
      expect(lock.expires_at).to be_within(2.seconds).of(now + 120.seconds)
    end

    it 'raises LockAdapterError on database failure' do
      allow(Kaal::CronLock).to receive(:create!).and_raise(StandardError, 'DB error')
  ...
  describe '#release' do
    ...
    it 'deletes lock record from database' do
      ...
      lock = Kaal::CronLock.find_by(key: 'test:release:456')
    ...
    it 'raises LockAdapterError on database failure' do
      adapter.acquire('test:key:error', 60)
      allow(Kaal::CronLock).to receive(:where).and_raise(StandardError, 'DB error')

      expect do
        adapter.release('test:key:error')
      end.to raise_error(Kaal::Backend::LockAdapterError, /SQLite release failed/)
    end
  end
  ...
  describe '#with_lock' do
    ...
    it 'releases lock after block execution' do
      ...
      lock = Kaal::CronLock.find_by(key: 'test:block:456')
      expect(lock).to be_nil
    end
    ...
    it 'releases lock even if block raises exception' do
      ...
      lock = Kaal::CronLock.find_by(key: 'test:block:789')
      expect(lock).to be_nil
    end
  ...
  describe 'Lock model' do
    it 'validates presence of key' do
      lock = Kaal::CronLock.new(key: nil, acquired_at: Time.current, expires_at: Time.current + 60)
      expect(lock).not_to be_valid
      expect(lock.errors[:key]).to be_present
    end

    it 'enforces unique constraint on key' do
      Kaal::CronLock.create!(key: 'unique:key', acquired_at: Time.current, expires_at: Time.current + 60)

      dup = Kaal::CronLock.new(key: 'unique:key', acquired_at: Time.current, expires_at: Time.current + 60)
      expect { dup.save!(validate: false) }.to raise_error(ActiveRecord::RecordNotUnique)
    end

    it 'validates acquired_at presence' do
      lock = Kaal::CronLock.new(key: 'test', acquired_at: nil, expires_at: Time.current + 60)
      expect(lock).not_to be_valid
      expect(lock.errors[:acquired_at]).to be_present
    end

    it 'validates expires_at presence' do
      lock = Kaal::CronLock.new(key: 'test', acquired_at: Time.current, expires_at: nil)
      expect(lock).not_to be_valid
    ...
    describe '.cleanup_expired' do
      it 'deletes expired locks' do
        ...
        Kaal::CronLock.create!(key: 'expired:1', acquired_at: past, expires_at: past + 5.seconds)
        Kaal::CronLock.create!(key: 'expired:2', acquired_at: past, expires_at: past + 5.seconds)

        deleted = Kaal::CronLock.cleanup_expired
      ...
      it 'does not delete valid locks' do
        ...
        Kaal::CronLock.create!(key: 'valid:1', acquired_at: now, expires_at: future)

        deleted = Kaal::CronLock.cleanup_expired
        expect(deleted).to eq(0)

        lock = Kaal::CronLock.find_by(key: 'valid:1')
        expect(lock).to be_present
      end
      ...
      it 'deletes only expired locks, keeps valid ones' do
        ...
        Kaal::CronLock.create!(key: 'expired', acquired_at: now - 10, expires_at: now - 5)
        Kaal::CronLock.create!(key: 'valid', acquired_at: now, expires_at: now + 100)

        deleted = Kaal::CronLock.cleanup_expired
        expect(deleted).to eq(1)

        expect(Kaal::CronLock.find_by(key: 'expired')).to be_nil
        expect(Kaal::CronLock.find_by(key: 'valid')).to be_present
      end
    end

    describe '#not_expired?' do
      it 'returns true for non-expired lock' do
        lock = Kaal::CronLock.new(
          key: 'test',
