...
RSpec.describe Kaal::Dispatch::DatabaseEngine do
  ...
  let(:cron_dispatch_class) { class_double(Kaal::CronDispatch) }

  before do
    stub_const('Kaal::CronDispatch', cron_dispatch_class)
  end

  describe '#log_dispatch' do
    it 'creates dispatch record in database' do
      fire_time = Time.current
      dispatch_record = instance_double(Kaal::CronDispatch)
