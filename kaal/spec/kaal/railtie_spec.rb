...
RSpec.describe Kaal::Railtie do
  before do
  ...
  describe '.ensure_logger!' do
    ...
    it 'does nothing when Rails.logger is nil' do
      ...
      expect(Kaal.configuration.logger).to be_nil
    end
  ...
  describe '.load_scheduler_file_on_boot!' do
    ...
    before do
      ...
      allow(Kaal).to receive(:logger).and_return(test_logger)
    end

    it 'loads scheduler file when file exists and policy is warn' do
      Kaal.configuration.scheduler_missing_file_policy = :warn
      Kaal.configuration.scheduler_config_path = 'config/scheduler.yml'
    ...
    it 'does not raise when file is missing and logger is nil' do
      Kaal.configuration.scheduler_missing_file_policy = :warn
      Kaal.configuration.scheduler_config_path = 'config/scheduler.yml'
      allow(Kaal).to receive(:logger).and_return(nil)
      allow(File).to receive(:exist?).with('/app/config/scheduler.yml').and_return(false)
    ...
    it 'loads scheduler file even when missing if policy is error' do
      Kaal.configuration.scheduler_missing_file_policy = :error
      Kaal.configuration.scheduler_config_path = 'config/scheduler.yml'
    ...
    it 'does not load scheduler file when path is blank and policy is warn' do
      Kaal.configuration.scheduler_missing_file_policy = :warn
      Kaal.configuration.scheduler_config_path = '   '
    ...
    it 'loads scheduler file for absolute scheduler path when file exists' do
      Kaal.configuration.scheduler_missing_file_policy = :warn
      Kaal.configuration.scheduler_config_path = '/tmp/scheduler.yml'
  ...
  describe '.register_signal_handlers' do
    ...
    it 'calls Kaal.stop! with timeout 30 when signal is received' do
      signal_blocks = []
      ...
      called_with = []
      allow(Kaal).to receive(:stop!) do |kwargs|
        called_with << kwargs
      end
    ...
    it 'logs the signal when logger is available' do
      ...
      allow(Kaal).to receive(:stop!).and_return(true)
    ...
    it 'rescues exceptions from stop! call in signal handler and logs them' do
      ...
      allow(Kaal).to receive(:stop!).and_raise(StandardError, 'Stop error')
    ...
    it 'logs warning when stop times out with logger present' do
      ...
      allow(Kaal).to receive(:stop!).and_return(false)
    ...
    it 'does not crash when stop times out with nil logger' do
      ...
      allow(Kaal).to receive_messages(logger: nil, stop!: false)
    ...
    it 'handles string command handler when logger is nil' do
      ...
      allow(Kaal).to receive_messages(logger: nil, stop!: true)
  ...
  describe 'Railtie class' do
    ...
    it 'registers rake task loader for kaal tasks' do
      original_rake = Rake.application
      Rake.application = Rake::Application.new

      Rails.application.load_tasks

      expect(Rake::Task.task_defined?('kaal:tick')).to be(true)
      expect(Rake::Task.task_defined?('kaal:status')).to be(true)
      expect(Rake::Task.task_defined?('kaal:explain')).to be(true)
      expect(Rake::Task.task_defined?('kaal:start')).to be(true)
    ensure
  ...
  describe '.handle_shutdown' do
    it 'stops scheduler when running' do
      ...
      allow(Kaal).to receive_messages(running?: true, logger: test_logger)
      allow(Kaal).to receive(:stop!).and_return(true)

      described_class.handle_shutdown

      expect(test_logger).to have_received(:info).with(/Rails is shutting down/)
      expect(Kaal).to have_received(:stop!).with(timeout: 10)
      expect(test_logger).not_to have_received(:warn)
    end

    it 'does nothing when not running' do
      allow(Kaal).to receive_messages(running?: false, logger: instance_spy(Logger))
      allow(Kaal).to receive(:stop!)

      described_class.handle_shutdown

      expect(Kaal).not_to have_received(:stop!)
    end

    it 'does not crash when logger is nil' do
      allow(Kaal).to receive_messages(running?: true, logger: nil)
      allow(Kaal).to receive(:stop!).and_return(true)
    ...
    it 'rescues exceptions from stop! in shutdown and logs them' do
      test_logger = instance_spy(Logger)
      allow(Kaal).to receive_messages(running?: true, logger: test_logger)
      allow(Kaal).to receive(:stop!).and_raise(StandardError, 'Shutdown error')
