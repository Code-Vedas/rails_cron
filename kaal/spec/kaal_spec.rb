...
RSpec.describe Kaal do
  ...
  describe '.reset_registry!' do
    ...
    it 'clears fallback definition registry when present' do
      fallback_registry = instance_double(Kaal::Definition::MemoryEngine)
  ...
  describe '.validate!' do
    it 'raises when invalid' do
      described_class.configuration.tick_interval = 0
      expect { described_class.validate! }.to raise_error(Kaal::ConfigurationError, /tick_interval/)
    end
  end
  ...
  describe '.register' do
    ...
    it 'raises RegistryError when key already exists' do
      ...
      expect do
        described_class.register(key: 'job:one', cron: '0 9 * * *', enqueue: ->(fire_time:, idempotency_key:) {})
      end.to raise_error(Kaal::RegistryError, /already registered/)
    end
    ...
    it 'applies :error by raising when key is sourced from scheduler file' do
      ...
      end.to raise_error(Kaal::RegistryError, /scheduler file/)
    end
    ...
    it 'raises SchedulerConfigError for unsupported conflict policy when key is sourced from scheduler file' do
      ...
      expect do
        described_class.register(key: 'job:file_defined', cron: '0 9 * * *', enqueue: ->(fire_time:, idempotency_key:) {})
      end.to raise_error(Kaal::SchedulerConfigError, /Unsupported scheduler_conflict_policy/)
    end
  ...
  describe '.definition_registry' do
    it 'uses backend provided definition registry when present' do
      backend = instance_double(Kaal::Backend::Adapter)
      definition_registry = instance_double(Kaal::Definition::Registry)
    ...
    it 'falls back to in-memory definition registry when backend returns nil' do
      ...
      expect(described_class.definition_registry).to be_a(Kaal::Definition::MemoryEngine)
    end
  ...
  describe '.coordinator' do
    it 'returns a Coordinator instance' do
      expect(described_class.coordinator).to be_a(Kaal::Coordinator)
    end
  ...
  describe '.reset_coordinator!' do
    it 'resets the coordinator and returns a new instance' do
      ...
      expect(new_coordinator).to be_a(Kaal::Coordinator)
    end
  ...
  describe '.dispatched?' do
    ...
    it 'returns false when adapter does not have dispatch_registry' do
      adapter = instance_double(Kaal::Backend::MemoryAdapter, respond_to?: false)
      described_class.configuration.backend = adapter
    ...
    it 'returns false when registry returns nil' do
      adapter = instance_double(Kaal::Backend::MemoryAdapter, dispatch_registry: nil)
      described_class.configuration.backend = adapter
    ...
    it 'returns the result from dispatch_registry.dispatched?' do
      registry = instance_double(Kaal::Dispatch::Registry)
      adapter = instance_double(Kaal::Backend::MemoryAdapter, dispatch_registry: registry)
      fire_time = Time.current
    ...
    it 'returns false and logs when adapter.dispatch_registry raises' do
      ...
      adapter = instance_double(Kaal::Backend::MemoryAdapter, respond_to?: true)
      fire_time = Time.current
  ...
  describe '.to_human' do
    ...
    it 'uses singular units for 1-minute intervals' do
      unit = Kaal::CronHumanizer.send(:interval_unit, 1, singular: 'minute', plural: 'minutes')
    ...
    it 'falls back when macro phrase mapping is unavailable' do
      stub_const('Kaal::CronHumanizer::MACRO_PHRASES', {})
      expect(described_class.to_human('@daily')).to eq('Cron: @daily')
    end

    it 'falls back when the humanized phrase is blank' do
      allow(Kaal::CronHumanizer).to receive(:humanize_expression).and_return('  ')
    ...
    it 'covers helper edge branches for interval and weekday extraction' do
      expect(Kaal::CronHumanizer.send(:derive_interval, [5, 10, 15])).to be_nil
      expect(Kaal::CronHumanizer.send(:derive_interval, [0, 0, 5])).to be_nil
      expect(Kaal::CronHumanizer.send(:derive_interval, [0, 0, 0])).to be_nil
      expect(Kaal::CronHumanizer.send(:derive_interval, [0, 10, 20])).to be_nil

      expect(Kaal::CronHumanizer.send(:extract_weekday, [1])).to eq(1)
      expect(Kaal::CronHumanizer.send(:extract_weekday, ['mon'])).to be_nil
      expect(Kaal::CronHumanizer.send(:extract_weekday, [['mon']])).to be_nil

      expect(Kaal::CronHumanizer.send(:weekday_name, 7)).to eq('Sunday')
    end
  end
end
