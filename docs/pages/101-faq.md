...
RailsCron.configure do |c|
  c.lock_adapter = RailsCron::Lock::RedisAdapter.new(Redis.new(url: "redis://localhost:6379/0"))
end
...
Check your `lock_adapter` configuration —
You must use **Redis** or **Postgres** for distributed safety.
The in-memory lock adapter only prevents duplicates _within the same process_.
