...
RSpec.describe RailsCron::Generators::InstallGenerator do
  ...
  describe '#install_migrations' do
    it 'copies sqlite migrations' do
      ...
      expect(generator).to have_received(:migration_template).with(
        'create_rails_cron_dispatches.rb.tt',
        'db/migrate/create_rails_cron_dispatches.rb'
      )
      expect(generator).to have_received(:migration_template).with(
        'create_rails_cron_locks.rb.tt',
        'db/migrate/create_rails_cron_locks.rb'
      )
      expect(generator).to have_received(:migration_template).with(
        'create_rails_cron_definitions.rb.tt',
        'db/migrate/create_rails_cron_definitions.rb'
      )
    end
  ...
  describe '#create_initializer' do
    it 'creates the RailsCron initializer' do
      generator = build_generator(backend: 'sqlite')
