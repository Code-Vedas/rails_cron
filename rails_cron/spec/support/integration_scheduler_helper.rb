...
module IntegrationSchedulerHelper
  ...
  def tick_scheduler(at:, coordinator: RailsCron.coordinator)
    travel_to(at) do
      coordinator.tick!
    end
  end
  ...
  def build_coordinator(adapter_instance:, registry:)
    ...
    RailsCron::Coordinator.new(configuration:, registry:)
  end
