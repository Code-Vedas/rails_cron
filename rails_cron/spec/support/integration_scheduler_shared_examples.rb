...
RSpec.shared_examples 'scheduler lifecycle integration' do
  ...
  it 'dispatches code-defined and file-defined jobs for the same tick' do
    ...
    expect(RailsCron.definition_registry.find_definition(code_key)).to include(source: 'code', enabled: true)
    expect(RailsCron.definition_registry.find_definition(file_key)).to include(source: 'file', enabled: true)
  end
  ...
  it 'does not dispatch disabled definitions and resumes after re-enable' do
    ...
    RailsCron.disable(key:)

    tick_scheduler(at: integration_tick_time)

    expect(recorder.snapshot).to be_empty
    expect(RailsCron.definition_registry.find_definition(key)).to include(enabled: false)

    RailsCron.enable(key:)
    ...
    expect(RailsCron.definition_registry.find_definition(key)).to include(enabled: true)
  end
  ...
  it 'dispatches again after the coordinator is rebuilt with the same callbacks' do
    ...
    original_coordinator = RailsCron.coordinator
    RailsCron.reset_coordinator!
    ...
    expect(RailsCron.coordinator).not_to be(original_coordinator)
...
RSpec.shared_examples 'shared store scheduler lifecycle integration' do
  ...
  it 'shares enabled definition state across coordinators backed by the same store' do
    ...
    registry_one = RailsCron::Registry.new
    registry_two = RailsCron::Registry.new
